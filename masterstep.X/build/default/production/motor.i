...
# 18 "motor.c" 2
# 50 "motor.c"
static volatile _Bool g_motorStepEnabled = 0;
...
static void MOTOR_ApplyEnablePin(_Bool enable);
static void MOTOR_ApplyDirectionPin(MOTOR_Direction_t direction);
static void MOTOR_ApplyStepGenerator(uint16_t frequencyHz, _Bool enable);
...
static void MOTOR_ApplyStepGenerator(uint16_t frequencyHz, _Bool enable){
    ...
    ticks = ((uint32_t)(489u)) / ((uint32_t)frequencyHz);
...
static uint16_t MOTOR_ClampStepFrequencyHz(uint16_t frequencyHz) {
    if (frequencyHz > (5000u)) return (5000u);
    if (frequencyHz == 0u) return 0u;
    if (frequencyHz < (1u)) return (1u);
    return frequencyHz;
}
...
void MOTOR_Initialise(MOTOR_t *motor){
    ...
    motor->direction = (MOTOR_DIRECTION_POSITIVE);
    ...
    g_motorTickCounter = 0u;
    g_motorStepIndex = 0u;
    g_motorDirection = (MOTOR_DIRECTION_POSITIVE);
    g_motorStepMode = MOTOR_STEP_MODE_HALF;

    MOTOR_AllOutputsOff();

    MOTOR_ApplyEnablePin(0);
    MOTOR_ApplyDirectionPin(motor->direction);
    MOTOR_ApplyStepGenerator(0u, 0);
...
void MOTOR_SetStepFrequencyHz(MOTOR_t *motor, uint16_t frequencyHz){
    ...
    if (motor->running) {
        ...
        else MOTOR_ApplyStepGenerator(motor->stepFrequencyHz, 1);
    }
}
...
void MOTOR_Start(MOTOR_t *motor){
    ...
    if(motor->stepFrequencyHz < (1u)) return;
    ...
    MOTOR_ApplyDirectionPin(motor->direction);
    MOTOR_ApplyStepGenerator(motor->stepFrequencyHz, 1);
