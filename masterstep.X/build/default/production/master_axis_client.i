...
# 2 "<built-in>" 2
# 1 "master_axis_client.c" 2
# 16 "master_axis_client.c"
# 1 "./master_axis_client.h" 1
# 17 "./master_axis_client.h"
...
_Bool MOTOR_IsEnabled(const MOTOR_t *motor);
...
_Bool MOTOR_IsRunning(const MOTOR_t *motor);
...
_Bool ENCODER_IsValid(const ENCODER_t *encoder);
...
typedef struct {
    ...
    _Bool homingReleaseMotionVerified;

    float homingSeekStartPosMm;
    _Bool homingSeekMotionVerified;

    int32_t homingLastRawCount;
    uint32_t homingLastMotionMs;
    _Bool homingStallDetectionArmed;
...
_Bool AXIS_IsHomed(const AXIS_t *axis);
_Bool AXIS_IsBusy(const AXIS_t *axis);
_Bool AXIS_HasFault(const AXIS_t *axis);
_Bool AXIS_IsEmergencyActive(const AXIS_t *axis);
# 19 "./slave_node.h" 2
...
_Bool MASTER_AXIS_CLIENT_SendCommand(AXIS_ID_t axisId, SLAVE_NODE_Command_t command, float value);
void MASTER_AXIS_CLIENT_Task(void);


_Bool MASTER_AXIS_CLIENT_IsBusy(AXIS_ID_t axisId);
_Bool MASTER_AXIS_CLIENT_HasFault(AXIS_ID_t axisId);
...
# 16 "master_axis_client.c" 2

# 1 "./uart_print.h" 1
# 36 "./uart_print.h"
extern const char charset[];

void send_uint8_as_hex_str_to_uart(uint8_t);
void send_uint16_as_hex_str_to_uart(uint16_t);
void send_uint32_as_hex_str_to_uart(uint32_t);
void send_uint16_as_str_to_uart(int16_t);
void send_int16_as_str_to_uart(int16_t);
void send_int32_as_str_to_uart(int32_t);
void send_string_to_uart(char *, uint8_t);
void send_string(char*);
void send_float_as_str_to_uart(float, uint8_t);

extern void (*UART_Write)(uint8_t);
# 17 "master_axis_client.c" 2
...
# 622 "./mcc_generated_files/pin_manager.h"
void IOCBF1_SetInterruptHandler(void (* InterruptHandler)(void));
# 646 "./mcc_generated_files/pin_manager.h"
extern void (*IOCBF1_InterruptHandler)(void);
# 670 "./mcc_generated_files/pin_manager.h"
void IOCBF1_DefaultInterruptHandler(void);
# 683 "./mcc_generated_files/pin_manager.h"
...
# 754 "./mcc_generated_files/pin_manager.h"
...
# 466 "./mcc_generated_files/tmr2.h"
void TMR2_Start(void);
# 495 "./mcc_generated_files/tmr2.h"
void TMR2_StartTimer(void);
# 527 "./mcc_generated_files/tmr2.h"
void TMR2_Stop(void);
# 559 "./mcc_generated_files/tmr2.h"
void TMR2_StopTimer(void);
# 594 "./mcc_generated_files/tmr2.h"
uint8_t TMR2_Counter8BitGet(void);
# 629 "./mcc_generated_files/tmr2.h"
uint8_t TMR2_ReadTimer(void);
# 668 "./mcc_generated_files/tmr2.h"
void TMR2_Counter8BitSet(uint8_t timerVal);
# 707 "./mcc_generated_files/tmr2.h"
void TMR2_WriteTimer(uint8_t timerVal);
# 759 "./mcc_generated_files/tmr2.h"
void TMR2_Period8BitSet(uint8_t periodVal);
# 811 "./mcc_generated_files/tmr2.h"
...
# 438 "./mcc_generated_files/uart1.h"
void UART1_SetOverrunErrorHandler(void (* interruptHandler)(void));
# 456 "./mcc_generated_files/uart1.h"
void UART1_SetErrorHandler(void (* interruptHandler)(void));
# 477 "./mcc_generated_files/uart1.h"
...
# 18 "master_axis_client.c" 2
# 29 "master_axis_client.c"
...
static _Bool MASTER_AXIS_CLIENT_IsAxisValid(AXIS_ID_t axisId) {
    return ((uint8_t)axisId < (uint8_t)AXIS_ID_COUNT);
}

static _Bool MASTER_AXIS_CLIENT_CommandMove(SLAVE_NODE_Command_t command) {
    return ((command == AXIS_NODE_CMD_MOVE_ABS_MM) ||
            (command == AXIS_NODE_CMD_MOVE_REL_MM));
}

static _Bool MASTER_AXIS_CLIENT_IsCommandValid(SLAVE_NODE_Command_t command) {
    switch (command) {
...
static _Bool MASTER_AXIS_CLIENT_WaitForAck(AXIS_ID_t axisId, uint8_t expectedSequence, CAN_PROTOCOL_ResponseFrame_t *response) {
    _Bool received;
    uint8_t waitCount;
    ...
    for (waitCount = 0u; waitCount < 50u; waitCount++) {
    ...
    _delay((unsigned long)((10u)*(16000000/4000.0)));
    }
    return 0;
}


static _Bool MASTER_AXIS_CLIENT_IsReadyForMove(AXIS_ID_t axisId) {
    uint8_t flags;

    if (!MASTER_AXIS_CLIENT_IsAxisValid(axisId)) return 0;

    flags = lastFlags[axisId];

    if ((flags & (1u << 0)) == 0u) return 0;
    if ((flags & (1u << 1)) != 0u) return 0;
    if ((flags & (1u << 2)) != 0u) return 0;
    if ((flags & (1u << 3)) != 0u) return 0;

    return 1;
}

static _Bool MASTER_AXIS_CLIENT_RequestStatusOnce(AXIS_ID_t axisId) {
    ...
    if (canResult != CAN_PROTOCOL_OK) return 0;
    if (MASTER_AXIS_CLIENT_WaitForAck(axisId, thisSequence, &responseFrame)) return 1;
...
static _Bool MASTER_AXIS_CLIENT_WaitUntilReadyForMove(AXIS_ID_t axisId) {
    uint8_t i;

    for(i = 0u; i < 120u; i++) {
        if(!MASTER_AXIS_CLIENT_RequestStatusOnce(axisId)) {
        _delay((unsigned long)((250u)*(16000000/4000.0)));
        continue;
        }
        if (MASTER_AXIS_CLIENT_IsReadyForMove(axisId)) return 1;
...
_Bool MASTER_AXIS_CLIENT_IsBusy(AXIS_ID_t axisId) {
    if (!MASTER_AXIS_CLIENT_IsAxisValid(axisId)) return 0;
    return ((lastFlags[axisId] & (1u << 1)) != 0u);
}

_Bool MASTER_AXIS_CLIENT_HasFault(AXIS_ID_t axisId) {
    if (!MASTER_AXIS_CLIENT_IsAxisValid(axisId)) return 1;
    return ((lastFlags[axisId] & (1u << 2)) != 0u);
}
