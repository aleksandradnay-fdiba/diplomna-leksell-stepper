...
static _Bool APP_CONFIG_IsDataValid(const APP_CONFIG_Data_t *config) {
    if (config == 0) return 0;

    if (config->magic != 0xCAFE) return 0;
    if (config->version != 1u) return 0;
    if (config->checksum != APP_CONFIG_CalculateChecksum(config)) return 0;

    if (config->encoderScaleMmPerCount <= 0.0f) return 0;
    if (config->maxTravelMm <= 0.0f) return 0;
    if (config->moveSpeedFastHz == 0u) return 0;
    if (config->moveSpeedSlowHz == 0u) return 0;
...
_Bool APP_CONFIG_Load(AXIS_t *axis, MOTOR_t *motor, ENCODER_t *encoder)
{
    ...
    if ((axis == 0) || (motor == 0) || (encoder == 0)) return 0;
    ...
    if (!APP_CONFIG_IsDataValid(&config)) return 0;
