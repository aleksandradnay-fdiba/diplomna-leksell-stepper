...
# 1 "./milliecond_timers.h" 1
# 46 "./milliecond_timers.h"
void millisecondEventHandler(void);
uint8_t setActiveTimersCount(uint8_t);


uint8_t timer(uint8_t, uint32_t);
void resetTimer(uint8_t);
uint32_t readTimer(uint8_t);
...
static _Bool AXIS_IsWithinTravelRange(const AXIS_t *axis, float positionMm);
static _Bool AXIS_IsConfigurationValid(const AXIS_Config_t *config);
...
static _Bool AXIS_IsEncoderMotionStopped(AXIS_t *axis);
...
static _Bool AXIS_HomingWaitDone(AXIS_t *axis);
...
static _Bool AXIS_IsWithinTravelRange(const AXIS_t *axis, float positionMm){
    ...
    if (positionMm < axis->config.minTravelMm) return 0;

    if (positionMm > axis->config.maxTravelMm) return 0;
...
static _Bool AXIS_IsConfigurationValid(const AXIS_Config_t *config) {
    ...
    if (config->stepsPerMm <= 0.0f) return 0;

    if (config->positionToleranceMm <= 0.0f) return 0;

    if (config->maxTravelMm <= config->minTravelMm) return 0;

    if (config->homingVerifyDistanceMm <= 0.0f) return 0;

    if (config->homingFinalBackoffMm <= 0.0f) return 0;

    if (config->homingRequiredDetections == 0u) return 0;

    if (config->homingTimeoutMs == 0u) return 0;
...
static _Bool AXIS_IsEncoderMotionStopped(AXIS_t *axis) {
    ...
    if((axis == 0) || (axis->encoder == 0)) return 0;
    ...
    if (axis->homingNoMotionTicks >= 20u) return 1;
...
static void AXIS_RunSimulation(AXIS_t *axis) {
    ...
    deltaMoveMm = speedMmPerSec * (0.01f);
...
static void AXIS_HandleHomingSeekReference(AXIS_t *axis) {
    ...
    if(seekTravelMm > axis->config.positionToleranceMm) {
        axis->homingSeekMotionVerified = 1;
        axis->homingStallDetectionArmed = 1;
    }
...
static void AXIS_HandleHomingReleaseFromReference(AXIS_t *axis) {
    ...
    if (releaseTravelMm > axis->config.positionToleranceMm) {
        axis->homingReleaseMotionVerified = 1;
    }
