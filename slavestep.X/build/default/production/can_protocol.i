...
static uint8_t CAN_PROTOCOL_BuildFlags(const SLAVE_NODE_Status_t *status) {
    ...
    if (status->homed) flags |= (1u << 0);
    if (status->busy) flags |= (1u << 1);
    if (status->faultActive) flags |= (1u << 2);
    if (status->emergencyActive) flags |= (1u << 3);
...
CAN_PROTOCOL_Result_t CAN_PROTOCOL_TryReadCommand (uint16_t expectedCanId,
    ...
    *received = 0;
    ...
    if (msg.frame.dlc != 8u) return CAN_PROTOCOL_ERROR_DLC;
