...
static volatile _Bool g_encoderPrevABValid = 0;

static _Bool ENCODER_IsScaleValid(const ENCODER_t *encoder);
...
static _Bool ENCODER_ReadHardwareRawCount(const ENCODER_t *encoder, int32_t *rawCount);
...
static _Bool ENCODER_PlatformReadRawCount(const ENCODER_t *encoder, int32_t *rawCount){
    (void)encoder;
...
static _Bool ENCODER_ReadHardwareRawCount(const ENCODER_t *encoder, int32_t *rawCount) {
    if((encoder == 0) || (rawCount == 0)) return 0;
...
void ENCODER_Initialise(ENCODER_t *encoder) {
    ...
    encoder->rawCount = (0);
    encoder->zeroOffset = (0);
    encoder->scaleMmPerCount = (0.002f);
    encoder->valid = 1;
    encoder->positionMm = (0.0f);
